...
/**
 * The main handler, used for spawning/handling wavify waves, as well as ticking the world's {@link WaterHandler}
 */
public class WavifyWaveHandler {
    ...
    public void spawnWaves(Set<BlockPos> waterBlocks) {
        ...
        for (BlockPos water : waterBlocks) {
            ...
            if (this.level.getBiome(spawnPos).is(BiomeTags.IS_RIVER)) bigWave = false;
...
//            if(site.xList.size() < 50) continue;
    ...
    /**
     * @return The wavify wave chunk radius - pulls from either the Wavify config or the server's render distance(whichever one is smaller).
     */
    ...
    /**
     * @return A random with a seed that will, most likely, be synced between clients despite being client side.
     */
