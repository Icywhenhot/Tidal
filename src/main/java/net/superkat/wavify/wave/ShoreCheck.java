...
public final class ShoreCheck {
    ...
    private static Walk walk(ClientLevel level, BlockPos from, float yaw, double sign, int steps) {
        ...
        for (int i = 0; i < steps; i++) {
            ...
            int stepX = Mth.floor(x);
            int stepZ = Mth.floor(z);
        ...
        for (int y = prefY + 1; y >= prefY - 2; y--) {
            for (int dx = -1; dx <= 1; dx++) {
                for (int dz = -1; dz <= 1; dz++) {
                    ...
                    if (!RiverFlow.isSurfaceWater(level, cursor)) continue;

                    double dist = Mth.square(cursor.getX() + 0.5 - x) + Mth.square(cursor.getZ() + 0.5 - z);
