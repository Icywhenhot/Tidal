...
public class StandingRiverWave extends Wave {
    ...
    public StandingRiverWave(ClientWorld world, RiverFlowPlanner.StandingWaveCandidate candidate, BlockPos anchorWater, int trainIndex) {
        super(world, anchorWater.up(), candidate.yaw(), 0.4f, false);
        ...
        this.motionPhase = this.world.random.nextFloat() * 20f + trainIndex * 2.5f;
        this.maxAge = 240 + this.world.random.nextInt(40);
    ...
    public float getRenderColumnWidth(int columnIndex, int columnCount) {
        float normalized = Math.abs(normalizedColumn(columnIndex, columnCount));
        return MathHelper.lerp(normalized, 0.7f, 1.0f);
    }

    @Override
    public float getRenderColumnLength(int columnIndex, int columnCount) {
        float normalized = Math.abs(normalizedColumn(columnIndex, columnCount));
        return this.length * MathHelper.lerp(normalized, 0.7f, 1.05f);
    }
    ...
    public void tick() {
        ...
        if (!WavifyWaveHandler.posIsWater(this.world, this.anchorWater) || !this.world.getBlockState(this.anchorWater.up()).isAir()) {
        ...
        float pulse = MathHelper.sin((this.age + this.motionPhase) * (0.24f + this.energy * 0.05f));
    ...
    private void sprayStandingCrest(float crest) {
        ...
        this.world.addParticleClient(
                ...
                splashX + this.world.random.nextGaussian() * 0.08f,
                this.y,
                splashZ + this.world.random.nextGaussian() * 0.08f,
                this.world.random.nextGaussian() * 0.025f,
                Math.abs(this.world.random.nextGaussian()) * 0.05f + 0.04f,
                this.world.random.nextGaussian() * 0.025f
        );
        this.world.addParticleClient(
